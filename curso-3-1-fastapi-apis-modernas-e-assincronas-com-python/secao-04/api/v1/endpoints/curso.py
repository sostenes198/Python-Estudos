...
async def post_curso(curso: CursoSchema, db: AsyncSession = Depends(get_session)) -> CursoSchema:
    ...
    novo_curso.titulo = curso.titulo
    novo_curso.aulas = curso.aulas
    novo_curso.horas = curso.horas
...
async def put_curso(curso_id: int, curso: CursoSchema, db:AsyncSession = Depends(get_session)) -> CursoSchema:
...
async def get_curso(curso_id: int, db: AsyncSession = Depends(get_session)) -> CursoSchema:
    async with db as session:
        query = select(CursoModel).filter(CursoModel.id == curso_id)
        result = await session.execute(query)
